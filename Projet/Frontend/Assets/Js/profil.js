...
async function saveEditField() {
  ...
  if (!value) {
    document.getElementById("message").textContent = "Veuillez entrer une valeur !";
  ...
  if (currentEditField === "username") {
    // Si la valeur n'a pas chager -> return
    ...
    // Mise à jour
    try {
      let result = await fetch("/api/editProfile", {
        method: "POST",
        headers: {
          "Content-Type": "application/json"
        },
      ...
      if (result.status === 200) {
        ...
        document.getElementById("message").style.backgroundColor = "rgb(122, 216, 122)";
        await new Promise(resolve => setTimeout(resolve, 2000));
        document.getElementById("message").style.display = "none";
        printInfos(profil);
        closeEditFieldModal();
  ...
  if (currentEditField === "emailConnect") {
    ...
    // Si l'adresse n'a pas changée -> return
    if (value === profil.email) {
      closeEditFieldModal();
      return;
    }
    ...
    if (!value.includes("@") || !value.includes(".")) {
      document.getElementById("message").textContent = "Format d'adresse courriel invalide !";
      document.getElementById("message").style.display = "block";
      document.getElementById("message").style.color = "red";
      await new Promise(resolve => setTimeout(resolve, 2000));
      document.getElementById("message").style.display = "none";
